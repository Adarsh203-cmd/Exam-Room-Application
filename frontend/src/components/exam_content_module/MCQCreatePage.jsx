...
const MCQCreatePage = () => {
  ...
  const removeOption = (option) => {
    setMcqData((prev) => {
      ...
      const filteredCorrectAnswers = prev.correct_answers.filter(
        (ans) => ans !== option
      );
      return {
        ...prev,
  ...
  const validate = () => {
    ...
    if (questionType === "MCQ") {
      ...
      if (mcqData.options.length < 2)
        newErrors.options = "Please add at least 2 options.";
      if (mcqData.correct_answers.length < 1)
        newErrors.correct_answers =
          "Please select at least one correct answer.";
      if (!mcqData.difficulty) newErrors.difficulty = "Difficulty is required.";
      if (!mcqData.marks || mcqData.marks < 1)
        newErrors.marks = "Marks must be at least 1.";
    } else {
      // FIB validations
      if (!fibData.subject) newErrors.subject = "Subject is required.";
      if (!fibData.question_text.trim())
        newErrors.question_text = "Question text is required.";
      if (!fibData.correct_answers.trim())
        newErrors.correct_answers = "Correct answer is required.";
      if (!fibData.difficulty) newErrors.difficulty = "Difficulty is required.";
      if (!fibData.marks || fibData.marks < 1)
        newErrors.marks = "Marks must be at least 1.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const url =
      questionType === "MCQ"
        ? "/api/exam_content/mcq/create/"
        : "/api/exam_content/fill/create/";
    ...
    <div style={styles.container}>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        {errors.question_text && (
          <div style={styles.errorText}>{errors.question_text}</div>
        )}
        ...
        {questionType === "MCQ" && (
          <>
            ...
            {errors.options && (
              <div style={styles.errorText}>{errors.options}</div>
            )}
            ...
            <div>
              {mcqData.options.map((option, index) => (
                <div key={index} style={styles.optionItem}>
                  ...
                  <input
                    type={
                      mcqData.answer_type === "Single" ? "radio" : "checkbox"
                    }
                    name="correct"
                    value={option}
            ...
            {errors.correct_answers && (
              <div style={styles.errorText}>{errors.correct_answers}</div>
            )}
        ...
        {questionType === "FIB" && (
          <>
            ...
            <textarea
              ...
              onChange={(e) =>
                setFibData({ ...fibData, correct_answers: e.target.value })
              }
        ...
        <select
          style={styles.select}
          value={
            questionType === "MCQ" ? mcqData.difficulty : fibData.difficulty
          }
          onChange={(e) =>
            questionType === "MCQ"
        ...
        {errors.difficulty && (
          <div style={styles.errorText}>{errors.difficulty}</div>
        )}
