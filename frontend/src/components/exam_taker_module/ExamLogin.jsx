  ...
  const validateExamTiming = async (examToken) => {
    try {
      ...
      const fifteenMinutesBeforeStart = new Date(
        examStartTime.getTime() - 15 * 60 * 1000
      );
      ...
      if (now < fifteenMinutesBeforeStart) {
        const timeUntilLogin = Math.ceil(
          (fifteenMinutesBeforeStart - now) / (1000 * 60)
        );
      ...
      if (now > examEndTime) {
        setTimeValidationError(
          `This exam has already ended. Exam ended at: ${formatExamTimeIST(
            exam.exam_end_time
          )}. ` +
      ...
      if (error.response?.status === 404) {
        setTimeValidationError(
          "Invalid exam code. Please check your exam URL and try again."
        );
      } else {
        setTimeValidationError(
          "Unable to verify exam timing. Please try again or contact support."
        );
      }
    } finally {
            ...
            <div className="signup-form outlined-form">
              <div style={{ textAlign: "center", padding: "2rem" }}>
                ...
                <h3 style={{ marginTop: "1rem", color: "#666" }}>
                  Validating Exam Schedule...
                </h3>
                <p style={{ color: "#888", fontSize: "14px" }}>
                  Please wait while we verify the exam timing.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="elogixa-signup-container">
        ...
        <div className="elogixa-signup-right">
          <div className="signup-form outlined-form">
            ...
            <p className="exam-login-subtitle">
              Please login to access your assigned exam
            </p>
            ...
            {timeValidationError && (
              <div
                className={`time-validation-message ${getTimeValidationClass()}`}
                style={{
                  padding: "1rem",
                  marginBottom: "1rem",
                  borderRadius: "8px",
                  backgroundColor: timeValidationError.includes(
                    "not yet available"
                  )
                    ? "#fff3cd"
                    : timeValidationError.includes("already ended")
                    ? "#f8d7da"
                    : "#f8d7da",
                  border: timeValidationError.includes("not yet available")
                    ? "1px solid #ffeaa7"
                    : "1px solid #f5c6cb",
                  color: timeValidationError.includes("not yet available")
                    ? "#856404"
                    : "#721c24",
                }}
              >
                <strong>
                  {timeValidationError.includes("not yet available")
                    ? "⏰ Too Early"
                    : timeValidationError.includes("already ended")
                    ? "⏰ Exam Expired"
                    : "❌ Access Denied"}
            ...
            <form onSubmit={handleLogin}>
              ...
              <div className="exam-token-display">
                <small>
                  Exam Code: <strong>{examToken}</strong>
                </small>
              </div>
              ...
              <div className="exam-login-info">
                <small>
                  <strong>Note:</strong> This login is specifically for your
                  assigned exam. Use the credentials provided to you by the
                  recruitment team.
                  {!timeValidationError && (
                    <span
                      style={{
            ...
            <div className="exam-login-footer">
              <small>
                Having trouble accessing your exam? Please contact the
                recruitment team for assistance.
              </small>
            </div>
          </div>
        </div>
