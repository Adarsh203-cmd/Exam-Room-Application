    ...
    const fetchExamData = async () => {
      ...
      try {
        ...
        questionsResponse.data.questions.forEach((question, index) => {
          ...
          initialAnswers[uniqueKey] = question.type === 'MCQ' 
            ? question.answer_type === 'multiple_select' ? [] : null 
        ...
        setTimeRemaining(`${initialHours.toString().padStart(2, '0')}:${initialMinutes.toString().padStart(2, '0')}:00`);
    ...
    try {
      ...
      try {
        ...
        const response = await axios.post(
          'http://127.0.0.1:8000/api/exam-view/submit-answers/',
          {
        ...
        if (submitError.response && 
          ...
          console.log("Exam was already submitted, redirecting to completion page");
          localStorage.removeItem('examAnswers');
          navigate('/exam-complete');
          return;
        }
      ...
      setError("Failed to submit exam. Your answers are saved locally. Please try again or contact support.");
      
      // If all else fails, just go to the completion page after confirmation
      if (window.confirm("There was an issue submitting your exam. Would you like to continue to the completion page anyway?")) {
        navigate('/exam-complete');
      }
    }
  };
