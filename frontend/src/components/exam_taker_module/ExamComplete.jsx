      ...
      try {
        ...
        try {
          evaluationResponse = await axios.post(
            `http://localhost:8000/api/evaluation/evaluate/${attemptId}/`,
            {},
            {
              headers: {
          ...
          if (evaluationError.response && 
              ...
              evaluationError.response.data.error.includes('duplicate key value violates unique constraint')) {
            
            console.log("Duplicate key error detected - result already exists, trying to fetch it");
            ...
            try {
              ...
              if (evaluationError.response.data.result_id) {
                ...
                const candidateResultsResponse = await axios.get(
                  `http://localhost:8000/api/evaluation/candidate-results/`, // You might need to pass candidate ID
                  {
                    headers: {
                      Authorization: `Bearer ${accessToken}`
                    }
                ...
                if (candidateResultsResponse.data && candidateResultsResponse.data.length > 0) {
              ...
              console.error("Failed to recover from duplicate key error:", fallbackError);
          ...
          while (retryCount < maxRetries) {
            try {
              resultResponse = await axios.get(
                `http://localhost:8000/api/evaluation/result/${resultId}/`,
                {
                  headers: {
                    Authorization: `Bearer ${accessToken}`
                  }
              ...
              retryCount++;
              console.log(`Result fetch attempt ${retryCount} failed:`, resultError);
        ...
        let errorMessage = "Failed to process exam results. Please contact support.";
        
        if (err.response?.data?.error) {
          if (err.response.data.error.includes('duplicate key')) {
            errorMessage = "Your exam has already been evaluated. Please contact support if you don't see your results.";
          } else {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-pink-100 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full mx-4 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Error Processing Results</h2>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full mx-4 text-center">
          ...
          <p className="text-gray-600 mb-6">Unable to retrieve your exam results.</p>
          <button
            onClick={handleBackToLogin}
            style={{
              backgroundColor: '#2563eb',
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-lg overflow-hidden mb-6">
            <div className={`p-6 ${getScoreBgColor(result.percentage_score)} border-l-4`}>
                    ...
                    {result.subjects.map((subject, index) => (
                      <div key={index} className="bg-gray-50 rounded-lg p-4">
                        ...
                        <div className="flex justify-between text-sm text-gray-600 mb-2">
                          <span>Marks: {subject.marks_obtained}/{subject.total_marks}</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full ${
                              subject.percentage >= 80 ? 'bg-green-500' :
                              subject.percentage >= 60 ? 'bg-blue-500' :
                              subject.percentage >= 40 ? 'bg-yellow-500' : 'bg-red-500'
                            }`}
          ...
          <div className="mt-8 text-center text-sm text-gray-500">
            ...
            <p>Evaluated on: {new Date(result.evaluated_at).toLocaleString()}</p>
          </div>
        </div>
      </div>
    </div>
